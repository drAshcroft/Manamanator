...
namespace MovieEditor.MyControls
{
    public partial class FrameSelector : UserControl
    {
       ...
       // private System.ComponentModel.Container components = null;
        ...
        private FMeasurementUnit measurementUnit = FMeasurementUnit.Second ;
        public FMeasurementUnit MeasurementUnit
        {
            get
            {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (this.orientation == Orientation.Horizontal)
            {
                e.Graphics.DrawLine(
                    ...
                    UI.ScaleWidth(LabelSize ), 
                    ClientRectangle.Top, 
                    UI.ScaleWidth(LabelSize ), 
                    ClientRectangle.Bottom);

                string abbStringName = MeasurementUnitDetails.Abbreviation (this.measurementUnit );
                string abbString = abbStringName;
            ...
            if (this.orientation == Orientation.Horizontal)
            {
                clipRegion.Exclude(new Rectangle(0, 0, UI.ScaleWidth(LabelSize+1 ), ClientRectangle.Height));
            }
            ...
            if (this.orientation == Orientation.Horizontal)
            {
                clipRegion.Exclude(new Rectangle(0, 0, UI.ScaleWidth(LabelSize +1), ClientRectangle.Height - 1));
            }
        ...
        private int[] GetSubdivs(FMeasurementUnit unit)
        {
            switch (unit)
            {
                case FMeasurementUnit.Second :
                {
                    return new int[] { 2, 5 };
                }

                case FMeasurementUnit.Frame :
                {
                    return new int[] { 2 };
                }
        ...
        private void FrameSelector_MouseMove(object sender, MouseEventArgs e)
        {
            //base.OnMouseMove(e);
        }
        ...
        {
            int ExcludeWidth = UI.ScaleWidth(LabelSize);
            float x = ((float)(e.X - ExcludeWidth)) / (float)dpu;
            Value = x;
            //base.OnMouseDown(e);
        }
    }

     public enum FMeasurementUnit   : int
       {
        Pixel = 1,
        Second = 2,
        Frame = 3
     ...
     {
         public static string Abbreviation(FMeasurementUnit Unit)
         {
             if (Unit == FMeasurementUnit.Pixel) return "Pix";
             if (Unit == FMeasurementUnit.Frame ) return "Frame";
             return "Sec";
         }
         public static double Conversion(FMeasurementUnit fromUnit, FMeasurementUnit toUnit)
         {
             return 1;
         }
         public static string Plural(FMeasurementUnit Unit)
         {
             if (Unit == FMeasurementUnit.Pixel) return "Pixels";
             if (Unit == FMeasurementUnit.Frame ) return "Frame";
             return "Seconds";
         }
   ...
   /// <summary>
    ...
    /// </summary>
     public struct ScaleFactor
     {
         ...
         private static string percentageFormat = "{0}%";
