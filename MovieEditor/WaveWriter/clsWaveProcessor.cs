...
public class clsWaveProcessor
{
    ...
    public void SaveWaveFile(string Filename,ref byte[] WaveData)
    {
        WaveHeaderOUT(Filename);
        WriteWAVEData(Filename, ref WaveData);
    }


    public BinaryWriter   CreateWaveFile(string strPath)
    {
        if (strPath == null) strPath = "";
    ...
    public bool ChangeVolume(string strPath, bool booIncrease, short shtPcnt)
    {
        ...
        for (int j = 0; j < arrfile.Length; j += 2)
        {
            ...
            if (newval !=null) //((newval[0] != null) && (newval[1] != null))
            {
                arrfile[j] = newval[0];
